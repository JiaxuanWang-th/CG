#!/usr/bin/env bash
# SPCF max cvm125：打包 test zip（75000 / 45000 / 90000）
set -euo pipefail
...
CKPT_DIR="experiments/straightpcf_max_cvm125"
SUMMARY="log/pack_spcf_max_cvm125_summary.txt"
if [[ -n "${MILESTONES:-}" ]]; then
  read -ra MILESTONES <<< "${MILESTONES}"
else
  MILESTONES=(75000 45000 90000)
fi
...
pack_one() {
  ...
  out_dir="results_spcf_max_cvm125_test_${tag}"
  zip="result_spcf_max_cvm125_${tag}.zip"
  pred_log="log/predict_spcf_max_cvm125_test_${tag}.log"
  task_yaml="configs/task/predict_spcf_max_cvm125_test_${tag}.yaml"
...
: > "$SUMMARY"
log "SPCF max cvm125 pack start | milestones: ${MILESTONES[*]}"
log "reference_online: ep97+SPCF75k=73.53 | CVM125=72.72"
